...
primary source of the timing discrepancy discussed in Section 5.
...
### 5.2 Per-validator latency distribution
...
## 12. Byzantine Fault Tolerance — k = 3 Corrupted Validators
...
### 12.2 Discovered Vulnerability — Timing Attack on Vote Collection
...
### 12.3 Protocol Fix — Collect All G Votes Before Aggregating
...
### 12.4 Results — All 5 Rounds Correct After the Fix
...
### 12.5 What the Frequency Values Reveal
...
### 12.7 Secondary Labels and Quorum Filtering
...
## 13. Byzantine Fault Tolerance — k = 6 Corrupted Validators (Beyond the BFT Bound)
...
### 13.3 What Empty Frequencies Mean at k = 6
...
### 13.4 Why `security` Still Cannot Reach Quorum
...
## 14. Architectural Optimization — Pre-computing LLM Inference at Mempool Entry
...
### 14.2 Independence of the Two LLM Steps
...
#### 14.4.4 Byzantine Timing Attack

The pre-computation model has a secondary security benefit. In the current protocol the timing vulnerability analyzed in Section 12.2 arises because Byzantine validators (which skip the LLM call) can submit their votes in milliseconds while honest validators are still waiting for Ollama. The G-threshold fix closes the attack, but the timing gap still exists.
...
### 14.5 Interaction with Mempool Transaction Volume
...
## 15. Labeling Latency Benchmark — Calibrating SelectTransactions Limits
...
### 15.3 Experiment A Results — Vary Transaction Count
...
### 15.4 Experiment B Results — Vary Prompt Length
...
### 15.5 What Drives LLM Labeling Latency
